...
# Navigate to the source_code directory inside the submission path using pushd
SOURCE_CODE_PATH="$SUBMISSION_PATH/source_code"
if [ ! -d "$SOURCE_CODE_PATH" ]; then
    echo "Error: The 'source_code' directory does not exist inside the submission path." >&2
    exit 1
fi
...
# Check if producer_consumer.c exists in the source_code directory
if [ ! -f "producer_consumer.c" ]; then
    echo "Error: producer_consumer.c not found in the source_code directory." >&2
    # Return to the previous directory using popd, suppress output
    popd > /dev/null
    exit 1
fi
